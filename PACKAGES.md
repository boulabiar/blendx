...
sources and uses statically linked Yoga and Blend2D.
...
containing Hermes, the N-API renderer, static Yoga, and static Blend2D.
...
|---|---|---|---|
| Yoga | pinned commit `433d4634d11c944bc9e64cfc90927fb10c0525b9` | Static, compiled into the Hermes host and test addon | Cached Flexbox measurement and layout for the retained native tree |
...
|---|---:|---|
| CMake | 3.31.6 observed; 3.20 minimum | Downloads verified source archives and configures Hermes, static Yoga/Blend2D/AsmJit, the host, and test addon |
| GCC/G++ | 15.2 | Compiles the host, renderer, Hermes, C++20 Yoga, and Blend2D |
...
Hermes, Yoga, Blend2D, AsmJit, and the Roboto asset are pinned by revision and SHA-256
in `CMakeLists.txt`. `HERMES_ROOT`, `BLEND2D_ROOT`, `ASMJIT_ROOT`, `YOGA_ROOT`,
and `BLENDX_FONT_FILE` are optional local-development overrides; normal builds and
CI do not depend on them. Roboto's complete OFL text is kept in
`LICENSES/Roboto-OFL.txt`; Yoga's MIT license is in `LICENSES/Yoga-MIT.txt`.
...
highlighting, diffs, canvas commands, virtualization, input widgets, or
